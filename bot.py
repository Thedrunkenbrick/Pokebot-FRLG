...
def find_image(file: str): # Function to find an image in a BizHawk screenshot
    try:
        ...
        template = cv2.imread(f"data/templates/{vers}/{lang}/" + file, cv2.IMREAD_UNCHANGED)
...
def mode_starters():
    if emu_info['detectedGame'] == "Pokemon Emerald (U)":
...
def mode_legendarys():
    debug_log.info(f"Soft resetting for a legendary...")
    release_all_inputs()

    while trainer_info["state"] != GameState.OVERWORLD and trainer_info["posX"] != 7 and trainer_info["posY"] != 13: press_button("A") 

    while trainer_info["state"] != GameState.MISC_MENU and trainer_info["mapBank"] != 0:

        press_button("A")
        if opponent_changed():
            if identify_pokemon(): input("Pausing bot for manual catch. Press Enter to continue...") 
