...
def ModeStarters():
    if any([x in GetEmu()["detectedGame"] for x in ["Emerald"]]):
        try:
        ...
        log.info("Getting to 1")
        ReleaseAllInputs()
        
        while GetTrainer()["state"] != GameState.OVERWORLD and GetTrainer()["mapBank"] == 0: 
            PressButton("A")
            
        
        while GetTrainer()["posY"] == 5 and len(GetParty(starter=True)) == 0:

            PressButton("A")
            
        while len(GetParty(starter=True)) > 0:
            try:
                if EncounterPokemon(starter=True): input("Pausing bot for manual catch (don't forget to pause bizhawk.lua script so you can provide inputs). Press Enter to continue...") # Kill bot and wait for manual intervention to manually catch the shiny starter
                else:
                    ResetGame()
